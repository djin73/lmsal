...
;
; now output to  text window determine if break required
...
if minprog then begin
   case dispopt of  
      2: begin
         update_piximg,  event.top
         
         widget_control, xstep_str.cimgbuts[3] + 16, SET_VALUE=pixpnt ;+16 because couldn't find a more consistent way to return the right event id
         widget_control,xstep_str.text.text,set_value=$
                            '  DISPLAYING  PIXMAP: FRAME ' + strtrim(pixpnt, 2)
      endcase
      else: begin
         update_image,event.top
         widget_control, xstep_str.cimgbuts[3] + 16, SET_VALUE=current
         widget_control,xstep_str.text.text,set_value=$
                            '  DISPLAYING  DATA: FRAME ' + strtrim(current, 2)
      endcase
   endcase   
...
;-----------Display Sequence Direction Function ------------------
	  ...
	  case dispopt of 
             2: begin
                update_piximg, event.top
                widget_control, xstep_str.cimgbuts[3] + 16, SET_VALUE=pixpnt
                widget_control,xstep_str.text.text,set_value=$
                            '  DISPLAYING  PIXMAP: FRAME ' + strtrim(pixpnt, 2)
             endcase
...
;     freeze removes update_image from background
      "Freeze": begin
          ...
          minprog=0
   ;       widget_control, 35, SET_VALUE=current
      endcase
;-----------------------------------------------------------------------
;
...
;-----------------------------------------------------------------------
      ...
      "SHIFT FRAME": begin
         ...
         fn = ''
         read, fn, prompt='Enter name of file to be saved to: '
         new = ''
         read, new, prompt='Create new file (y) or use existing file (n)? [y/n] '
...
;     	files - strarray of data file names
...
;
; ------------------------------------------------------------------------
;| QUIT | FWRD>> | <<BWRD | Movie | Freeze | AZoom-OFF | Wrap |
;XLOADCT | SHIFT FRAME   | 
; ------------------------------------------------------------------------
...
event_options= $
   ['QUIT','FWRD>>','<<BWRD','Movie','Freeze',$
	'AZoom-OFF','Wrap','XLOADCT', 'SHIFT FRAME']
...
slide_options = ['FRAME', 'FRAMERATE']
slide=widget_slider(basex,/suppres,/drag,  $
;	title='<<<START' + spaces + ' FRAME' + spaces + 'END>>>', $ 
                    font=butfont, maximum = ((size(cube))[3] - 1), minimum = 0, uval = 'frame')
slabel=widget_label(basex,/frame,font=butfont, $
                    value='<<<START(0)' + spaces + ' FRAME NUMBER' + spaces + 'END('+ strtrim(((size(cube))[3] - 1),2)+')>>>')

slide2=widget_slider(basex,/suppres,/drag, $ 
                    font=butfont, value=30, maximum=60, minimum = 0, uval='rate')

slabel2=widget_label(basex,/frame,font=butfont, $
   value='<<<SLOWER(1)' + spaces + ' FRAMERATE (FPS)' + spaces + 'FASTER(60)>>>')
...
;help, multi_draw_str.menubuts
;print, multi_draw_str.menubuts
;help, buttons
